...
AutoGearOverrideSpecs = {
	{
		["label"] = "Ascension",
		["subLabels"] = {"None", "Strength", "Agility", "Intellect", "Spirit"}
	},
	{
		["label"] = "Death Knight",
...
local function OptionsSetup(optionsMenu)
	...
	frame[i]:SetText("AutoGear ASC "..agVersion)
...
optionsMenu:SetScript("OnEvent", function (self, event, arg1, ...)
	if event == "PLAYER_ENTERING_WORLD" then
		...
		AutoGearOptions = {
			...
			{
				["option"] = "ShowBestReward",
				["cliCommands"] = { "reward", "rewards" },
				["cliTrue"] = { "enable", "on", "start" },
				["cliFalse"] = { "disable", "off", "stop" },
				["label"] = "Mark best Reward on Quest Dialog",
				["description"] = "Marks best Reward on Quest Dialog",
				["toggleDescriptionTrue"] = "Mark best Reward is now enabled.",
				["toggleDescriptionFalse"] = "Mark best Reward is now disabled."
			},
			{
				["option"] = "StickToWeaponChoice",
				["cliCommands"] = { "stick" },
				["cliTrue"] = { "enable", "on", "start" },
				["cliFalse"] = { "disable", "off", "stop" },
				["label"] = "Stick to Weapon Choice",
				["description"] = "Stick to current weapon choice. i.e. don't equip a 2H if a 1H is equipped, and no wand if bow is equipped.",
				["toggleDescriptionTrue"] = "Stick to Weapon Choice is now enabled.",
				["toggleDescriptionFalse"] = "Stick to Weapon Choice is now disabled."
			},
			{
			...
			},
			{
				...
				["shouldUse"] = ((PawnGetDefaultScale ~= nil) and PawnGetDefaultScale()),
				...
				["child"] = {
					["option"] = "PawnScale",
					["options"] = (function() if PawnGetDefaultScale ~= nil and PawnGetDefaultScale() then return AutoGearGetPawnScales() end end)(),
...
SlashCmdList["AutoGear"] = function(msg)
	...
	(param1 == "scaleoverride")) then
		if PawnGetDefaultScale ~= nil and PawnGetDefaultScale() then
...
--AutoGearPrint("AutoGear: "..event..(arg1 and " "..tostring(arg1) or "")..(arg2 and " "..tostring(arg2) or "")..(arg3 and " "..tostring(arg3) or "")..(arg4 and " "..tostring(arg4) or ""), 0)
	...
	if (AutoGearDB.AutoAcceptQuests and not CallBoardUI:IsVisible()) then
...
function GetCurrentTypes()
	local id1 = GetInventoryItemID("player", GetInventorySlotInfo("MainHandSlot"))
	local itemdata1 = GetItemInfoInstant(id1)
	local id2 = GetInventoryItemID("player", GetInventorySlotInfo("SecondaryHandSlot"))
	local itemdata2 = GetItemInfoInstant(id2)
	
	return itemdata1['subclassID'], itemdata2['subclassID']
end

function WeaponTypeToString(weaponType)
end

function GetCurrentTypesString(slot)
	
	local slotinfo
	
	if slot == "MainHandSlot" then
		slotinfo = GetInventoryItemID("player", GetInventorySlotInfo("MainHandSlot"))
	elseif slot == "SecondaryHandSlot" then
		slotinfo = GetInventoryItemID("player", GetInventorySlotInfo("SecondaryHandSlot"))
	elseif slot == "RangedSlot" then
		slotinfo = GetInventoryItemID("player", GetInventorySlotInfo("RangedSlot"))
	end
...
function GetWeaponTypeString()
	...
	if link then
		...
		if itemdata then
			--print("itemid: "..tonumber(string.match(link, ":(%d+)")))
			local classid = itemdata['classID']
			local subclassid = itemdata['subclassID']
...
function ReadItemInfo(inventoryID, lootRollID, container, slot, questRewardIndex, link)
	...
	--local currentMainHandType, currentOffHandType = GetCurrentTypes()
	local weaponTypeString = GetWeaponTypeString()
	--print(weaponTypeString)
	...
	for i = 1, AutoGearTooltip:NumLines() do
		...
		if (mytext) then
			...
			local getText = mytext:GetText();
			if(getText == nil) then getText = "0,0" end
			local text = select(1,string.gsub(getText:lower(),",",""));
			if (i==1) then
			...
			-----------------------WEAPON HANDLING START
			
			if (text=="main hand") then
				if (weapons == "dagger and any" and weaponType ~= 15) then
					cannotUse = 1
			...
			if (text=="off hand") then
				...
				elseif (weapons == "weapon and shield" and weaponType ~= 6) then
					cannotUse = 1
					reason = "(this spec needs a shield in the off-hand)"
				elseif (weapons == "dual wield" and CanDualWield() and weaponType == 6) then
					cannotUse = 1
			...
			-- stick to "types" start
			
			if weaponType then
			
				if info.Slot then
					if AutoGearDB.StickToWeaponChoice then
						if GetCurrentTypesString(info.Slot) ~= GetWeaponTypeString() then
							cannotUse = 1
							reason = "(other weapon type)"
						end
					end
				end
			
				--AutoGearPrint("AutoGear:         weaponTypeReturn: "..weaponType, 2)
				if (string.find(text, "damage per second")) then 
					info.DPS = (info.DPS or 0) + value 
					--AutoGearPrint("AutoGear:         dps: "..info.DPS, 2)
				end
				...
				if (minDamage and maxDamage) then
					...
					--AutoGearPrint("AutoGear:         damage: "..minDamage.." - "..maxDamage, 2)
					minDamage, maxDamage = nil
				end
			else
				--AutoGearPrint("AutoGear:         weaponType: none", 2)
			end
			...
			if ((g==0 or r/g>3) and (b==0 or r/b>3) and math.abs(b-g)<0.1 and r>0.5 and mytext:GetText()) then --this is red text
				...
				cannotUse = 1
				if(string.find(mytext:GetText(), VANITY_NOT_OWNED)) then
					cannotUse = nil
					reason = ""
				end
			end
		...
		if (rightText) then
			...
			if ((g==0 or r/g>3) and (b==0 or r/b>3) and math.abs(b-g)<0.1 and r>0.5 and rightText:GetText()) then --this is red text
				...
				cannotUse = 1
				if(string.find(rightText:GetText(), VANITY_NOT_OWNED)) then
					cannotUse = nil
					reason = ""
				end
			end
		...
		if info.Slot then
			local itemId, unknown = GetInventoryItemID("player", GetInventorySlotInfo(info.Slot))
			if itemId then
				local itemdata = GetItemInfoInstant(itemId)
				if itemdata then
					if itemdata["quality"] == 7 and UnitLevel("player") ~= MAX_PLAYER_LEVEL_TABLE[GetAccountExpansionLevel()] then
						cannotUse = 1
						reason = "(wearing a heirloom)"
					end
				end
			end
		end
	...
	if (AutoGearDB.UsePawn == true) and (PawnGetDefaultScale ~= nil) and PawnGetDefaultScale() then
...
function GetWeaponType()
	...
	local name, link = AutoGearTooltip:GetItem()
	if link then
		--AutoGearPrint("AutoGear:         weaponType: "..link, 2)
		local itemdata = GetItemInfoInstant(tonumber(string.match(link, ":(%d+)")))
		...
		--AutoGearPrint("AutoGear:         classID: "..itemdata['classID'], 2)
		--AutoGearPrint("AutoGear:         subclassID: "..itemdata['subclassID'], 2)
		if itemdata then
			if (itemdata['classID'] == 2) or ((itemdata['classID'] == 4) and (itemdata['subclassID'] == 6)) then
				return itemdata['subclassID']
			end
		end
		--AutoGearPrint("AutoGear:         no itemdata", 2)
	end
end
...
function AutoGearTooltipHook(tooltip)
	...
	if (tooltipItemInfo.shouldShowScoreInTooltip == 1 and tooltipItemInfo.Slot) then
